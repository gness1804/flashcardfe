...
...
  ...
  bindCallApply: {
    id: 26,
    question: 'What are the Apply, Call and Bind methods? Why are they an essential part of JS?',
    ...
    firstPoint: 'Bind is most often used to set the value of "this" for a function.',
    secondPoint: 'The Call method is very similar to Bind, except that the former calls the function automatically.',
    thirdPoint: 'Apply allows us to execute a function with an array of parameters, rather than a set of individual arguments. Apply can work well with a variadic function (one with a varying number of arguments).',
  ...
  dataBinding: {
    ...
    main: 'With one-way data flow, the model (data store) is the single source of truth: the model alone can change the app\'s state, though the UI can signal the intent to make such a change. With two-way data binding, the model and the UI are dependent on each other such that when one changes, the other changes accordingly.',
    firstPoint: 'Two-way data binding is particularly important in AngularJS, while React relies on a form of one-way data flow.',
    firstLinkName: 'Stack Overflow post explaining two-way data binding.',
    firstLinkURL: 'https://stackoverflow.com/questions/13504906/what-is-two-way-binding',
  },
  shimming: {
    id: 28,
  ...
  storage: {
    ...
    firstPoint: 'Cookies are an older mechanism for storing data involving headers sent from the server. They are still widely used, but they are vulnerable to certain kinds of security risks and can be more cumbersome than the other two forms of storage.',
    secondPoint: 'localStorage and sessionStorage both involve browser storage of data in key-value pairs. They are very similar except that localStorage persists across sessions while sessionStorage is gone when a session is ended (the browser is closed).',
    firstLinkName: 'Turing School Lesson on storage types',
  ...
  equality: {
    ...
    main: 'The == is a "loose" equality, which involves type conversion. (Example: 2 == "2" is true). The ===, by contrast, compares two values without type conversion, so it is stricter. (2 === "2" is false).',
